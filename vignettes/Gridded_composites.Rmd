...
library(compositeR)
library(lipdR)
library(ens) # ensemble methods (split out of geoChronR)
library(lipdViz) # visualization (split out of geoChronR)
library(ggplot2)
library(dplyr)
library(tidyr)
library(purrr)
...
anim <- baseMap(spatialMedians$lon, lat = spatialMedians$lat, f = .2) +
  geom_tile(data = spatialMedians, 
            aes(x = lon, y = lat, fill = composite),
            alpha = .5, 
            color = "black") +
  scale_fill_distiller(palette = "RdBu", 
                       limit = max(abs(spatialMedians$composite),
                                   na.rm = TRUE) * c(-1, 1)) +
...
animate(anim)
```
### Gridded composites through time
...
```{r,eval=FALSE}
#some additional code that is useful to save the animation
